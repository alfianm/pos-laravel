    ...
    <div class="relative group">
        @if($icon)
            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <span class="text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                    {!! $icon !!}
                </span>
            </div>
        @endif
        ...
        <input 
            ...
            {{ $attributes->whereDoesntStartWith(['model', 'label', 'error', 'helpText', 'icon', 'type', 'isCurrency'])->class([
